...
describe("FormCTA Component", () => {
  ...
  test("CT01-02: Nome preenchido", () => {
    ...
    fireEvent.click(screen.getByLabelText(/enviar/i));
    expect(
      screen.queryByText("O campo Nome é obrigatório.")
  ...
  test("CT02-01: Email vazio", async () => {
    ...
    fireEvent.input(screen.getByLabelText(/empresa/i), {
      target: { value: "Minha Empresa" },
    });
    fireEvent.input(
      screen.getByLabelText(/Nos fale mais sobre o seu negócio./i),
    ...
    await waitFor(() => {
      const errorMessage = screen.getByText(
        "Por favor, preencha os seguintes campos: email"
      );
  ...
  test("CT02-03: Email válido", () => {
    ...
    fireEvent.click(screen.getByLabelText(/enviar/i));
    expect(
      screen.queryByText("O campo Email é obrigatório.")
    ).not.toBeInTheDocument();
    expect(
  ...
  test("CT03-01: Exibição de snackbar ao tentar enviar formulário com campos vazios", async () => {
    ...
    render(<LottieButton onSubmit={mockOnSubmit} formState={{ name: '',  }} />);
    fireEvent.click(screen.getByLabelText('enviar'));
    expect(screen.getByText(/Por favor, preencha os seguintes campos:/)).toBeVisible();
  });
